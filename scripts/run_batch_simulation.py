...
OUTPUT_PATH = PROJECT_ROOT / "data" / "processed" / "simulation_results"
...
def main() -> None:
    ...
    spark = create_spark_session("RunBatchSimulation")
    ...
    try:
        ...
        selected_output_df = simulation_df.select(
            "features",
            "label",
            "prediction",
            "fraud_probability",
        ...
        selected_output_df.write.mode("overwrite").parquet(str(OUTPUT_PATH))

        print("Simulation result preview:")
        selected_output_df.show(10, truncate=False)

        print("Aggregated simulation outcomes:")
        engine.aggregate_results(selected_output_df).show(truncate=False)

        metrics = calculate_business_metrics(
            simulation_df=selected_output_df,
